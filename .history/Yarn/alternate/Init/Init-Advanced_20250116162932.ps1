...
function Init-YarnProject {#+
    ...
    # Create a background job that waits for input and times out after specified interval#-
    $job = Start-Job -ScriptBlock {#-
      param ($prompt, $timeout)#-
      Read-Host -Prompt "$prompt" | Out-null; return (Read-Host)#-
  } `#-
     -ArgumentList @($Prompt, ($Timeout *1000)) `#-
     | Receive-Job `#-
     | Where { $_ } |#-
     ForEach { return $_ }#-
   ...
   if ((Get-Job).State.ToString() –eq 'Completed'){#-
       stop-job ((get-job))#-
   }#-
   else{#-
        stop-job ((get-job))#-
...
>>>>>>> Tabnine >>>>>>># {"conversationId":"2f0cae92-5d25-4975-9a81-eb05dd35eb8b","source":"instruct"}
